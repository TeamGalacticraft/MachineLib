...
minecraft {
    ...
    runs {
        ...
        create("gameTestServer") { // name must match exactly for options to be applied, apparently
            workingDirectory(project.file("run"))
            args("-mixin.config=${modId}.mixins.json", "-mixin.config=${modId}.forge.mixins.json", "-mixin.config=machinelib_test.mixins.json")
            ideaModule("${rootProject.name}.${project.name}.test")
            taskName("runGametest")
            sources(sourceSets.main.get(), sourceSets.test.get())
            property("forge.enabledGameTestNamespaces", "machinelib_test,minecraft") // minecraft because forge patches @GameTest for the filtering... and common cannot implement the patch
            mods {
                create(modId) {
                    source(sourceSets.main.get())
                    source(project(":Common").sourceSets.main.get())
